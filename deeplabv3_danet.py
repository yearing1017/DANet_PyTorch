...
from DAN_V3_V0420.danet import DANet
model_urls = {
...
class ResNet(nn.Module):
    def __init__(self, block, block_num, num_classes, num_groups=None, weight_std=False, beta=False, pretrained=False):
        ...
        self.conv2 = self.conv(num_classes * 2, num_classes, kernel_size=1, stride=1, bias=False)
        # 遍历模型进行初始化
    ...
    def forward(self, x):
        ...
        # danet返回的结果是一个list
        x_dan = self.danet(x_res)

        out = torch.cat((x_aspp, x_dan),1)
