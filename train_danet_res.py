...
def train(epoch = 400):
    ...
    # 定义最好指标miou数值，初始化为0
    best_pred = 0.0
    writer = SummaryWriter('tblog/danet_v0408')
    # 指定第二块gpu
    ...
    for epo in range(epoch):
        ...
        for i in range(5):
            ...
            for index, (image, label) in enumerate(train_dataloader[i]):
                ...
                if np.mod(index, 24) == 0:
                    ...
                    print(line)
                    # 写到日志文件
                    with open('log/logs_danetv0408.txt', 'a') as f :
                        f.write(line)
                        f.write('\r\n')
        ...
        if new_pred > best_pred:
            best_pred = new_pred
            torch.save(danet_model.state_dict(), 'models_danetv0408/danet_{}.pth'.format(epo))
