...
def train(epoch = 400):
    ...
    # 定义最好指标miou数值，初始化为0
    best_pred = 0.0
    writer = SummaryWriter('tblog/danet_v3_0420')
    # 指定第二块gpu
    ...
    danet_model = resnet152()
    danet_model = danet_model.to(device)
    ...
    optimizer = optim.Adam(danet_model.parameters(), lr=1e-3)
    ...
    for epo in range(epoch):
        ...
        for i in range(5):
            ...
            for index, (image, label) in enumerate(train_dataloader[i]):
                ...
                if np.mod(index, 24) == 0:
                    ...
                    print(line)
                    # 写到日志文件
                    with open('log/logs_danet_v3_0420.txt', 'a') as f :
                        f.write(line)
                        f.write('\r\n')
        ...
        if new_pred > best_pred:
            best_pred = new_pred
            torch.save(danet_model.state_dict(), 'models_danet_v3_0420/danet_{}.pth'.format(epo))
